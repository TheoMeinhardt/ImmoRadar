...
async function addUser(newUser: user): Promise<user | undefined> {
  ...
  const params = [newUser.username, newUser.firstname, newUser.middlename, newUser.lastname, newUser.addressID, newUser.company, newUser.phone, newUser.email, null, newUser.profilePic, newUser.password];
...
async function patchUser(id: string, data: user): Promise<user> {
  ...
  const params = [data.username, data.addressID, data.company, data.phone, data.email, data.profilePic, data.password, data.firstname, data.middlename, data.lastname, id];
